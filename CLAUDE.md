...
### 明示的な LINE 指示は高リスク制限をスキップ

LINE ユーザーが以下の操作を**明示的にコマンドとして送信した場合**は、確認なしで即実行してよい:

| 操作 | LINE コマンド例 |
|---|---|
| `git commit` | 「コミットして」「コミット: 〇〇」 |
| `git push` | 「プッシュして」 |
| `git pull` | 「プルして」 |
| `git merge` | 「mainをマージして」 |
| `npm publish` | 「パブリッシュして」 |

> **根拠**: LINE 経由のコマンドはユーザーが明示的に意図して送信したものであり、Claude が自律的に判断して実行するケースとは異なる。

### 高リスク（実行禁止 → pending-actions.md へ記録）

以下は **Claude が自律的に判断して実行しようとした場合** に限り、`pending-actions.md` に追記して終了すること:
